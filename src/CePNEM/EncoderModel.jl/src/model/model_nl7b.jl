function generate_model_nl7b(xs_s, list_θ, ewma_trim, idx_splits)
    x1 = xs_s[1,:] # velocity
    x2 = xs_s[2,:] # θh
    x3 = xs_s[3,:] # pumping
    x5 = xs_s[4,:] #curvature
    ...
    return function (ps)
        ewma((sin(ps[1]) .* (1 .- 2 .* lesser.(x1, list_θ[1])) .+ cos(ps[1])) .*
            (ps[2] .* x1 .+
                ...
                ps[4] .* x3 .+
                ps[5] .* x4 .+
                ps[6]), ps[7], ewma_trim, idx_splits) .+ ps[8]
    end
end
...
5: curvature
"""
function generate_model_nl7b_partial(xs_s, idx_valid, list_θ, ewma_trim, idx_splits)
    x1 = xs_s[1,:] # velocity
    x2 = xs_s[2,:] # θh
    x3 = xs_s[3,:] # pumping
    x5 = xs_s[4,:] #curvature
...
function init_ps_model_nl7b_component(xs, idx_component=1:5)
    ps_0 = []
    ps_min = []
    ps_max = []
    list_idx_ps = []
